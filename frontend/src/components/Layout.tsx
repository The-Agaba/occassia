...
import { Activity, CalendarDays, FileText, LogOut, Menu, Settings2, ShieldCheck, UserRound, X } from 'lucide-react';
...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <aside className={`app-sidebar ${open ? 'is-open' : ''}`}>
      ...
      <nav className="app-nav">{visibleNav.map(({ to, label, note, icon: Icon }) => { const active = location.pathname.startsWith(to); return <Link key={to} to={to} onClick={(event) => { event.preventDefault(); setOpen(false); navigate(to); }} className={`app-nav-link ${active ? 'active' : ''}`}><span className="nav-icon"><Icon size={18} /></span><span><b>{label}</b><small>{note}</small></span></Link>; })}</nav>
