...
export default function Layout({ children }: { children: React.ReactNode }) {
  ...
  return <div className="app-shell">
    ...
    <aside className={`app-sidebar ${open ? 'is-open' : ''}`}>
      ...
      <nav className="app-nav">{visibleNav.map(({ to, label, note, icon: Icon }) => { const target = to === '/card-control' ? (currentEventId ? `/events/${currentEventId}/cards` : '/card-control') : to; const active = to === '/card-control' ? location.pathname.includes('/cards') || location.pathname === '/card-control' : location.pathname.startsWith(to); return <Link key={to} to={target} onClick={() => setOpen(false)} className={`app-nav-link ${active ? 'active' : ''}`}><span className="nav-icon"><Icon size={18} /></span><span><b>{label}</b><small>{note}</small></span></Link>; })}</nav>
